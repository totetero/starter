...
for ARG in "$@" ; do
	...
	case $ARG in
		...
		sync_get|get)
			...
			;;
		setup)
			docker exec -it ${DOCKER_CONTAINER_NAME_01} bash -c 'cd /root/'${PROJECT}'/'${APP}' && bundle install'
			;;
		serve)
			docker exec -it ${DOCKER_CONTAINER_NAME_01} bash -c 'cd /root/'${PROJECT}'/'${APP}' && bundle exec rails server'
			;;
		browse)
