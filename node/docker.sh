...
for ARG in "${@}" ; do
	...
	case ${ARG} in
		...
		install)
			...
			;;
		build_cli)
			docker exec -it ${DOCKER_CONTAINER} /bin/bash -c 'source bin/profile.sh && npm run build_cli'
			;;
		build_srv)
			docker exec -it ${DOCKER_CONTAINER} /bin/bash -c 'source bin/profile.sh && npm run build_srv'
			;;
		serve)
