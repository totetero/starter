...
for ARG in "${@}" ; do
	...
	case ${ARG} in
		...
		build_ts)
			...
			;;
		build_go)
			docker exec -it ${DOCKER_CONTAINER} /bin/bash -c 'source bin/profile.sh && GOOS=js GOARCH=wasm go build -o public/index.wasm src/client/main.go'
			;;
		serve)
			docker exec -it ${DOCKER_CONTAINER} /bin/bash -c 'source bin/profile.sh && go run src/server/main.go'
			;;
		serve_address)
