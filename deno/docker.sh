...
for ARG in "${@}" ; do
	...
	case ${ARG} in
		...
		sync_get|get)
			...
			;;
		welcome)
			docker exec -it ${TARGET1_CONTAINER} /bin/bash -c 'source bin/profile.sh && deno run https://deno.land/std/examples/welcome.ts'
			;;
		serve)
			docker exec -it ${TARGET1_CONTAINER} /bin/bash -c 'source bin/profile.sh && deno run --allow-net --allow-read src/main.ts'
			;;
		serve_address)
