...
for ARG in "${@}" ; do
	...
	case ${ARG} in
		...
		build)
			...
			;;
		serve)
			docker exec -it ${TARGET1_CONTAINER} /bin/bash -c 'source bin/profile.sh && npm run start -- --host 0.0.0.0'
			;;
		clean)
